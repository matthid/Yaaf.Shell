...
module StreamModule =
    type StreamHelper(istream:IStream<byte array>) =
        ...
        member x.ReadAsync (dst, offset, count) = 
            read dst offset count
        member x.WriteAsync (dst, offset, count) = 
            write dst offset count
        override x.Flush () = ()        
    ...
    module StreamExtensions = 
        type System.IO.Stream with
            ...
            member s.AsyncRead c = AsyncStreamHelper<_>.FromAdvancedRead (s.MyReadAsync) c
            member s.AsyncRead (buffer, offset, count) = s.MyReadAsync(buffer, offset, count)
            member s.AsyncWrite (buffer, offset, count) = s.MyWriteAsync(buffer, offset, count)
        type IStream<'a> with
            member s.ReadWait() = 
    ...
    let toInterface buffersize (stream:System.IO.Stream) = 
        ...
        let write src = async {
            do! stream.MyWriteAsync(src, 0, src.Length)
            stream.Flush()
            }
        let dispose () = 
